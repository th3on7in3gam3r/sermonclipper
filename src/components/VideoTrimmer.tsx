...
export default function VideoTrimmer({ initialFile, onTrimComplete, onCancel }: VideoTrimmerProps) {
  ...
  const trimSegment = async (start: number, end: number) => {
    ...
    try {
      ...
      if (trimmedFile.size > MAX_DIRECT_UPLOAD_BYTES) {
        toast.error(
          `Segment is ${Math.round(trimmedFile.size / (1024 * 1024))}MB — over the ${MAX_DIRECT_UPLOAD_MB}MB limit. Try the next segment or use manual trim.`,
          { id: trimToast }
        );
        setIsTrimming(false);
        return;
      ...
      toast.success(
        `Trimmed ${formatTime(trimDuration)} (${Math.round(trimmedFile.size / (1024 * 1024))}MB) — uploading...`,
        { id: trimToast }
      );
  ...
  const handleTrim = async () => {
    ...
    if (isOverLimit()) {
      toast.error(`Selected range is still over ${MAX_DIRECT_UPLOAD_MB}MB. Shorten it.`);
      return;
    }
    ...
    try {
      ...
      const jobId =
        typeof crypto !== 'undefined' && crypto.randomUUID
          ...
          : Date.now().toString(36);
      toast.success(
        `Trimmed to ${formatTime(trimDuration)} (${Math.round(trimmedFile.size / (1024 * 1024))}MB)`
      );
      onTrimComplete(trimmedFile, jobId);
      await ffmpeg.deleteFile('input.mp4');
      await ffmpeg.deleteFile('output.mp4');
      ...
      >
        <div style={{ width: '100%', maxWidth: '500px', textAlign: 'center' }}>
          ...
          <p style={{ color: '#A1A1AA', fontSize: '16px', marginBottom: '32px' }}>
            Select a large sermon video. We&apos;ll help you split it into uploadable segments.
          </p>
          <div
            onClick={() => document.getElementById('trimmer-input')?.click()}
            style={{
              padding: '48px',
          ...
          >
            ...
            <p style={{ color: '#52525B', fontSize: '15px', marginTop: '8px' }}>
              Any size — stays on your device
            </p>
          </div>
          <input
            id="trimmer-input"
        ...
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            ...
            <h2 style={{ fontSize: '24px', fontWeight: 900, marginBottom: '8px' }}>
              Your sermon is {Math.round(file.size / (1024 * 1024))}MB
            </h2>
            <p style={{ color: '#A1A1AA', fontSize: '16px', lineHeight: 1.6 }}>
              We&apos;ve split it into {segments.length} segments of ~
              {formatTime(segments[0]?.end - segments[0]?.start)} each.
              <br />
              Click <b style={{ color: '#C4B5FD' }}>✂️ USE THIS</b> to trim & upload that segment instantly,
              or <b style={{ color: '#A1A1AA' }}>⚙</b> to fine-tune the in/out points.
            </p>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: '16px', color: '#71717A', marginBottom: '12px' }}>
                    ~{seg.sizeMB}MB · {formatTime(seg.end - seg.start)} long
                  </div>

                  {isProcessing ? (
                    <div>
                      <div
                        style={{ fontSize: '15px', color: '#C4B5FD', fontWeight: 800, marginBottom: '6px' }}
                      >
          ...
          {ffmpegLoading && (
            <p style={{ textAlign: 'center', marginTop: '20px', color: '#8B5CF6', fontSize: '16px' }}>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', gap: '10px' }}>
            <span style={{ fontSize: '16px', color: '#34D399', fontWeight: 800, fontFamily: 'monospace' }}>
              IN {formatTime(inPoint)}
            </span>
            <span style={{ fontSize: '16px', color: '#F87171', fontWeight: 800, fontFamily: 'monospace' }}>
              OUT {formatTime(outPoint)}
            </span>
          </div>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <span style={{ fontSize: '16px', color: '#71717A' }}>
              Duration: <b style={{ color: '#C4B5FD' }}>{formatTime(outPoint - inPoint)}</b>
            </span>
        ...
        >
          <div
            style={{
              ...
              background: isOverLimit() ? 'rgba(239,68,68,0.1)' : 'rgba(139,92,246,0.15)',
        ...
        <button
          ...
          style={{
            ...
            fontSize: '15px',
            fontWeight: 900,
            opacity: isTrimming || !ffmpegLoaded || isOverLimit() ? 0.5 : 1,
            cursor: isTrimming || !ffmpegLoaded || isOverLimit() ? 'not-allowed' : 'pointer',
          }}
        >
          {isTrimming
            ? `TRIMMING... ${trimProgress}%`
            : !ffmpegLoaded
              ? 'Loading Engine...'
              : isOverLimit()
                ? `TOO LARGE (${getEstimatedSize()}) — Shorten Range`
                : `✂️ TRIM & UPLOAD (${getEstimatedSize()})`}
        </button>
