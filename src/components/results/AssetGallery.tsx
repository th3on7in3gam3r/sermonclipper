...
function AssetCard({ asset, type }: AssetCardProps) {
  ...
  const prompt =
    (asset as SermonImage & QuoteVerse).full_image_prompt ||
    (asset as SermonImage & QuoteVerse & { prompt?: string; image_prompt?: string }).prompt ||
    (asset as SermonImage & QuoteVerse & { image_prompt?: string }).image_prompt;
  const title = (asset as SermonImage).title || (asset as QuoteVerse).text;
  const subtitle =
    (asset as SermonImage).description || (asset as QuoteVerse).reference || (asset as QuoteVerse).type;
  ...
  return (
    <div
      className={`glass-card p-8 space-y-6 border-t-4 ${type === 'sermon' ? 'border-amber-500' : 'border-violet-500'} relative overflow-hidden group bg-zinc-900/40 border-x border-b border-white/5`}
    >
      {generatedImage ? (
        ...
        <div className="aspect-[9/16] w-full rounded-2xl bg-zinc-950 flex flex-col items-center justify-center space-y-4 border border-white/5 animate-pulse">
          <div
            className={`w-10 h-10 border-4 ${type === 'sermon' ? 'border-amber-500' : 'border-violet-500'} border-t-transparent rounded-full animate-spin`}
          />
          <p className="text-sm font-black uppercase tracking-[0.2em] text-zinc-500 text-center px-8 leading-relaxed">
            DALL-E is crafting your {type} masterpiece...
          </p>
        </div>
      ) : (
        <div
          className="aspect-[9/16] w-full rounded-2xl bg-zinc-950 flex flex-col items-center justify-center space-y-6 border border-white/5 border-dashed group-hover:border-white/20 transition-all cursor-pointer"
          onClick={handleGenerate}
        >
          <div
            className={`w-16 h-16 rounded-full flex items-center justify-center transition-all group-hover:scale-110 shadow-2xl ${type === 'sermon' ? 'bg-amber-600 shadow-amber-600/20' : 'bg-violet-600 shadow-violet-600/20'}`}
          >
          ...
          <div className="text-center">
            <span className="text-sm font-black text-zinc-600 uppercase tracking-[0.3em] block">
              Generate Design
            </span>
          </div>
        </div>
      ...
      <div className="space-y-4 relative z-10">
        <div className="space-y-1">
          <span
            className={`text-sm font-black uppercase tracking-[0.4em] ${type === 'sermon' ? 'text-amber-500/60' : 'text-violet-500/60'}`}
          >
        ...
        {subtitle && (
          <p className="text-sm text-zinc-500 leading-relaxed italic border-l-2 border-white/5 pl-4">
            {subtitle}
          </p>
        )}
      </div>
