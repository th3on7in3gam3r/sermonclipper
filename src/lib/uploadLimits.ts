...
export function formatUploadLimitError(sizeBytes: number): string {
  ...
  return `File is ${sizeMb}MB — maximum direct upload is ${MAX_DIRECT_UPLOAD_LABEL}. Convert your YouTube link to MP4 or MP3 (e.g. media.ytmp3.gg), compress with HandBrake if needed, then upload here.`;
}
