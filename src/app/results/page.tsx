  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      try {
        ...
        if (!res.ok) {
          console.error(`OpenAI route returned ${res.status}`);
          return;
        }
          ...
          <div className="clip-preview">
            {videoUrl ? (
              ...
              <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.1 }}>
                <p style={{ fontSize: '10px', fontWeight: 900, textTransform: 'uppercase' }}>Full Session</p>
              </div>
            )}
            <div style={{ position: 'absolute', bottom: '12px', left: '12px', background: 'rgba(0,0,0,0.7)', padding: '4px 12px', borderRadius: '100px', fontSize: '10px', fontWeight: 900 }}>FULL SESSION</div>
          </div>
          <div className="clip-info">
            <h3 style={{ fontWeight: 900, fontSize: '18px' }}>Master Sermon Session</h3>
            <p style={{ color: '#555', fontSize: '12px' }}>
              {analysis?.summary || 'High-resolution session capture. Perfect for full-length archiving.'}
            </p>
          </div>
        </div>
          ...
          [1, 2, 3].map((i) => (
            <div key={i} className="clip-card" style={{ opacity: 0.5 }}>
              <div className="clip-preview vertical" style={{ background: 'rgba(255,255,255,0.03)' }}>
                <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                  <p style={{ fontSize: '11px', color: '#666' }}>HARVESTING CLIP {i}...</p>
                </div>
              </div>
            </div>
          ))
