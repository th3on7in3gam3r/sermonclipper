      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '120px 24px 80px', position: 'relative', zIndex: 1 }}>
        ...
        {loading ? (
          ...
          <>
            ...
            {team.isOwner && (
              <div className="glass-card premium-border" style={{ padding: '24px', marginBottom: '24px' }}>
                <h3 style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.1em', marginBottom: '16px' }}>INVITE MEMBER</h3>
                <input
                  type="email"
                  placeholder="colleague@church.org"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', color: '#fff', marginBottom: '12px' }}
                />
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value as 'editor' | 'viewer')}
                  style={{ width: '100%', padding: '12px', borderRadius: '12px', marginBottom: '16px', background: '#212130', color: '#fff', border: '1px solid rgba(255,255,255,0.1)' }}
                >
                  <option value="editor">Editor — create and export clips</option>
                  <option value="viewer">Viewer — view clips only</option>
                </select>
                <button type="button" className="vesper-btn vesper-btn-primary shimmer-effect" onClick={invite}>
                  Send invite
                </button>
              </div>
            )}
            ...
            <div className="glass-card premium-border" style={{ padding: '24px' }}>
              <h3 style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.1em', marginBottom: '16px' }}>MEMBERS</h3>
              {team.members.map((m) => (
                <div key={m.email} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <div>
                  ...
                  {team.isOwner && m.role !== 'owner' && (
                    <button type="button" className="vesper-btn-outline" style={{ color: '#EF4444' }} onClick={() => removeMember(m.email)}>
                      Remove
                    </button>
                  )}
                </div>
              ...
              {team.pendingInvites.length > 0 && (
                <>
                  <h4 style={{ marginTop: '20px', fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em' }}>PENDING</h4>
                  {team.pendingInvites.map((i) => (
                    <p key={i.email} style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '8px' }}>
                      {i.email} ({i.role})
                    </p>
                  ))}
                </>
