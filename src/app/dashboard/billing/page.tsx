  ...
  return (
    <main className="vesper-mesh-bg-container" style={{ minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '120px 24px 80px', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginBottom: '32px' }}>Manage your plan, payment method, and invoices via Stripe.</p>
          ...
          <div className="glass-card premium-border" style={{ padding: '28px' }}>
            <div style={{ marginBottom: '24px' }}>
              ...
              {summary.nextBillingDate && (
                <p style={{ color: 'var(--text-muted)', marginTop: '8px' }}>Next billing date: {summary.nextBillingDate}</p>
              )}
            ...
            <button type="button" className="vesper-btn vesper-btn-primary shimmer-effect" onClick={openPortal} style={{ width: '100%', marginBottom: '12px' }}>
              Upgrade or change plan
            </button>

            {summary.paymentMethod?.last4 && (
              <div style={{ padding: '16px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', marginBottom: '16px' }}>
                <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '6px' }}>Payment method</p>
                <p style={{ fontWeight: 800 }}>
                  Card ending in {summary.paymentMethod.last4} · exp {summary.paymentMethod.exp}
                </p>
                <button type="button" className="vesper-btn-outline" style={{ marginTop: '12px' }} onClick={openPortal}>
                  Update card
                </button>
              </div>
            )}

            <h3 style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.1em', marginBottom: '12px' }}>INVOICE HISTORY</h3>
            {summary.invoices.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>No invoices yet.</p>
            ) : (
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {summary.invoices.map((inv) => (
                  <li key={inv.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', borderRadius: '10px', background: 'rgba(255,255,255,0.02)' }}>
                    <span>
                      {inv.date} · {inv.amount}
                    </span>
                    {inv.pdfUrl && (
                      <a href={inv.pdfUrl} target="_blank" rel="noreferrer" className="vesper-btn-outline" style={{ padding: '6px 12px', fontSize: '11px', textDecoration: 'none' }}>
                        PDF
                      </a>
                    )}
                  </li>
            ...
            <button
              ...
              style={{ marginTop: '32px', background: 'none', border: 'none', color: 'var(--text-dim)', fontSize: '13px', cursor: 'pointer', textDecoration: 'underline' }}
            >
              Cancel subscription
            </button>
          </div>
        ...
        <div className="upgrade-modal-overlay" role="presentation" onClick={() => setShowCancelModal(false)}>
          <div className="upgrade-modal-card glass-card premium-border" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="upgrade-modal-copy">
              You will keep access until the end of your billing period, then your account downgrades to Free (2 clips/month, watermarked exports).
              Export history and your clip library remain available.
            </p>
            <button type="button" className="vesper-btn vesper-btn-primary upgrade-modal-cta" onClick={openPortal}>
              Continue in Stripe
            </button>
            <button type="button" className="vesper-btn-outline upgrade-modal-cta" style={{ marginTop: '10px' }} onClick={() => setShowCancelModal(false)}>
              Keep subscription
            </button>
          </div>
        </div>
